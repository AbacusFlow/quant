...
- **Ledger locking** — `output/.ledger.lock` serializes all read-modify-write of `executions.csv`/`signal_log.csv`: daily_signal.py holds it around holdings-read→log-append→write_planned; record_trade.py takes `.daily_local.lock` (orchestration layer, shared with daily_local.sh) **then** `.ledger.lock` — always in that order (daily_local.sh's child daily_signal only takes the ledger lock, so no deadlock). `daily_signal.account_state()` returns the real replayed (total equity, cash); buy sizing uses total equity (fallback `--capital`), buy suggestions are capped by available cash + same-run sell proceeds, and orders smaller than `REBALANCE_BAND × equity` are suppressed (matching the backtest engine's band; full liquidations exempt). Ledger integrity errors (parse failure/negative cash/negative position/non-finite or garbage amounts) **propagate and fail the signal run** (fail-closed) rather than silently degrading.
