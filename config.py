...
ROTATION_BUFFER = 0.02      # 换仓缓冲:新标的动量需超过当前持仓 2% 才切换
                            # 2026-08-13 由 0.01 调至 0.02,依据 research/2026-08-13-rotation-buffer.md
                            # (复现脚本 scripts/experiment_buffer.py / experiment_buffer_robust.py)。
                            # 决策依据仅为「摩擦下降」(全期):一日游抖动 -53%、换手 -21%、
                            # 佣金/滑点/总摩擦各 -11%。
                            # 收益差不显著(block bootstrap 95%CI 含 0,仍不能排除约 1.6pp 年化劣化),
                            # 不得以「能多赚」为由;滚动窗口 78% 胜率因窗口高度重叠,仅为描述性统计。
                            # 注意:近两年抖动改善几乎不存在(抖=1 仅 10→9),改善主要来自 2018-2022 震荡年;
                            # 短期感知会明显弱于全期。纪律:勿因短期表现反复回调(简化走查规则劣于固定值)。
